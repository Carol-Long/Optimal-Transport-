...
X = ot.bregman.barycenter_stabilized(A, M, reg, weights)

# non-covid
examples1 = len(training_non)
weights1 = np.array([1/examples1 for i in range(examples1)]) #weights1 n*1
Y = ot.bregman.barycenter_stabilized(A1, M, reg, weights1)

X = np.reshape(X,(k,k))
Y = np.reshape(Y,(k,k))

np.savetxt("bregmanbarystabCovid.csv", X, delimiter=",")
np.savetxt("bregmanbarystabNon.csv", Y, delimiter=",")
...
#tp,fp,tn,fn
f.write("Bregmanbary Stabilized Results:\n")
